...
export default function Home() {
  ...
  return <div className="site-shell">
    ...
    <main id="atas">
      <section className="hero"><div className="hero-copy reveal"><p className="eyebrow">Satu hari yang kami simpan</p><h2>Untuk dirayakan<br /><i>bersama.</i></h2><p className="hero-intro">Dengan penuh syukur dan bahagia, kami mengundang Anda untuk hadir menjadi bagian dari awal halaman baru kami.</p><div className="hero-countdown" aria-label="Hitung mundur menuju hari pernikahan"><span className="eyebrow">Menuju 19 Juni 2027</span><div>{[[remaining.days, "hari"], [remaining.hours, "jam"], [remaining.minutes, "menit"], [remaining.seconds, "detik"]].map(([value, label]) => <div key={label}><strong>{String(value).padStart(2, "0")}</strong><small>{label}</small></div>)}</div></div><a className="text-link" href="#cerita">Baca kisah kami <ArrowDown size={15} /></a></div><div className="hero-photo reveal"><img src="/manus-storage/arsip-bunga-hero_e1b8984e.jpg" alt="Meja bunga, kertas, dan cincin pernikahan" /><span className="photo-note">Bandung · Jawa Barat<br />sebuah sore, 2026</span></div></section>
